...
//  advent
//
//  Created by pankaj pal on 20/12/19.
...
static const array<uint8_t, 4> directions = {
    1,  // North
    2,  // South
    3,  // West
    4   // East
...
void ProblemSolver(const vector<long>& vect) {
    Computer computer;
    computer.vect = vect;
    computer.setInput(0);
    ...
    Point pos {0, 0};
    Point oxyPoint;
    int64_t oxyPathLen = 0;
    int64_t movablePoints = 0;
    set<Point> visited;
    set<Point> walls;

    std::function<Point(Point, int64_t)> GetNewPos = [](Point pos, int64_t direction) {
        if (direction == 1) {
            --pos.y;
        }
    ...
    auto MoveDroid = [&](int64_t direction) {
        computer.setInput(direction);
        return computer.calculate();
    };

    // DFS avoids the need to maintain computer state at each point
    // BFS just messes it up
    std::function<void(int64_t)> TryDirection = [&](int64_t direction) {
        auto newPos = GetNewPos(pos, direction);

        if (visited.count(newPos) || walls.count(newPos)) {
            return;
        }

        auto result = MoveDroid(direction);
        ...
        // Backtrack
        pos = GetNewPos(pos, Opposite(direction));
        MoveDroid(Opposite(direction));
    };
    ...
    cout << "Found oxygen point at " << oxyPoint.x << "," << oxyPoint.y << "with length of path as: " << oxyPathLen <<endl;

    // Problem2
    set<Point> oxygenated;
    uint64_t maxDepth = 0;

    function<void(Point, uint64_t)> OxyGenate = [&](Point p, uint64_t depth) {
        if (oxygenated.count(p) > 0 || walls.count(p) > 0) {
            return;
        }
