...
//  advent
//
//  Created by pankaj pal on 13/12/19.
...
// It seems to be too much fun to try it out programmatically, hence doing it manually :)
void Problem2(vector<long>& vect) {
    vect[0] = 2;
    Computer computer;
    computer.vect = vect;

    string grid = "B,C,B,A,B,A,C,A,A,C";
    string A = "R,4,L,10,R,10";
    string B = "L,4,L,4,L,10,R,4";
    string C = "R,4,L,4,L,4,R,8,R,10";

    vector<long> inputVector;

    for (auto c : grid) {
        inputVector.push_back((int)c);
    }
    ...
    inputVector.push_back((int)'y');
    inputVector.push_back(10);

    int index = 0;
    long dust = 0;
    while(!computer.done) {
        computer.setInput(inputVector[index]);
        dust = computer.calculate();
        index++;
    }

    cout << "Problem2: " << dust << endl;
}
...
void Problem1(const vector<long>& vect) {
    ...
    while (!computer.done) {
        char ch = (char)computer.calculate();

        if (result.empty()) {
            std::vector<char> _newvec;
            _newvec.push_back(ch);
            result.push_back(_newvec);
        } else if (ch == '\n') {
            std::vector<char> _newvec;
            result.push_back(_newvec);
        } else {
    ...
    for (int r = 1; r < result.size()-1; r++) {
        for (int c = 1; c < result[r].size()-1; c++) {
            if (result[r].empty())
                break;
...
int main(int argc, const char * argv[]) {
    ...
    // Add extra elements to input to avoid index overshooting to undefined memory locations
