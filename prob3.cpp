...
map< pair <int, int>, int > get_path(vector<string> line)
{
    ...
    for (auto _line : line) {
        char dir = _line[0];
        int pos = std::stoi(_line.substr(1, _line.length()-1));
...
int main(int argc, const char * argv[]) {
    ...
    map<std::pair<int, int>, int > line1 = get_path(vect1);
    map<std::pair<int, int>, int > line2 = get_path(vect2);

    set<pair<int, int>> set1;
    set<pair<int, int>> set2;

    for (map<pair<int, int>, int>::iterator it = line1.begin(); it != line1.end(); ++it) {
        pair<int, int> pair = it->first;
        set1.insert(make_pair(pair.first, pair.second));
    }
    for (map<pair<int, int>, int>::iterator it = line2.begin(); it != line2.end(); ++it) {
        pair<int, int> pair = it->first;
        set2.insert(make_pair(pair.first, pair.second));
    }
    ...
    for(auto &&n : v_intersection) {
        int distance1 = abs(n.first) + abs(n.second);
        if (distance1 < min1) {
            min1 = distance1;
        }

        //cout << n.first << ":" << n.second << endl << line1[n] << endl << line2[n] << endl;
        int distance2 = line1[n] + line2[n];
        if (distance2 < min2) {
            min2 = distance2;
        }
    }

    cout << "Problem1:" << min1 << endl;
    cout << "Problem2:" << min2 << endl;
